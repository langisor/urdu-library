        ...
        <div className="grid grid-cols-2 gap-4">
          {currentQuestion.options.map((option, index) => (
            <Card
              ...
              className={`cursor-pointer transition-colors duration-200 ${feedback ? (option.text === currentQuestion.correctAnswer ? 'border-green-500' : 'border-red-500') : ''}`}
            >
              <CardContent className="flex flex-col items-center p-4">
                ...
                <span className="text-sm font-medium">{option.phonetic}</span>
              </CardContent>
            </Card>
          ))}
        </div>
