...
export function LessonDashboard({
  ...
  return (
    <div className="flex flex-col items-center text-right" dir="rtl">
      ...
      <div className="w-full mb-6"><Header categoryName={categoryName} categoryID={lessons[0].categoryID} /></div>
      {/* Lessons Grid */}
      <BackToUnit /> 
          ...
          <div key={lesson.id} className="flex flex-col gap-2  w-full sm:w-120">
            <Card dir="rtl" className="mt-4 w-full">
              <CardContent className="p-2 h-full">
                <h2 className="text-2xl font-bold mt-2 text-center mb-5">{lesson.name}</h2>
                <Progress value={lesson.countQuiz} />
                {/* icons */}
                <div className="w-full flex gap-2 mx-10">
                  <div className="w-full flex gap-4 mt-8 items-center">
                    <span className="rounded-lg text-blue-600 bg-gray-100 p-1"><Pen className="h-6 w-6" /></span>
                    <span className="text-md">{lesson.countQuiz}</span>
                  </div>
                  <div className="w-full flex gap-4 mt-8 items-center">

                    <span className="rounded-lg text-blue-600 bg-gray-100 p-1"><TextCursor className="h-6 w-6" /></span>
                    <span className="text-md">{lesson.countWords}</span>
                  </div>
                  <div className="w-full flex gap-4 mt-8 items-center">

                    <span className="rounded-lg text-blue-600 bg-gray-100 p-1"><SquaresIntersect className="h-6 w-6" /></span>
